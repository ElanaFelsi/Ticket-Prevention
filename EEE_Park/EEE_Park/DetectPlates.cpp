...
std::vector<PossibleChar> findPossibleCharsInScene(cv::Mat &imgThresh) {
    ...
    cv::findContours(imgThreshCopy, contours, cv::RetrievalModes::RETR_LIST, cv::ContourApproximationModes::CHAIN_APPROX_SIMPLE);        // find all contours
...
PossiblePlate extractPlate(cv::Mat &imgOriginal, std::vector<PossibleChar> &vectorOfMatchingChars) {
    ...
    cv::Point2f p2dPlateCenter(dblPlateCenterX, dblPlateCenterY);

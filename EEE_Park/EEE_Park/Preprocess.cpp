...
void preprocess(cv::Mat &imgOriginal, cv::Mat &imgGrayscale, cv::Mat &imgThresh) {
    ...
    cv::adaptiveThreshold(imgBlurred, imgThresh, 255.0, cv::AdaptiveThresholdTypes::ADAPTIVE_THRESH_GAUSSIAN_C, cv::ThresholdTypes::THRESH_BINARY_INV, ADAPTIVE_THRESH_BLOCK_SIZE, ADAPTIVE_THRESH_WEIGHT);
}
...
cv::Mat maximizeContrast(cv::Mat &imgGrayscale) {
    ...
    cv::morphologyEx(imgGrayscale, imgTopHat, cv::MorphTypes::MORPH_TOPHAT, structuringElement);
    cv::morphologyEx(imgGrayscale, imgBlackHat, cv::MorphTypes::MORPH_BLACKHAT, structuringElement);

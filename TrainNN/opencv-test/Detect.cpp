...
static vector<string> list_folder(string path)
{
...
static vector<string> list_file(string folder_path)
{
	vector<string> files;
...
string SVMPredict() {
	...
	cvtColor(image, gray, IMREAD_GRAYSCALE);
	//imshow("gray", gray);
	cvtColor(image, binary, IMREAD_GRAYSCALE);
	std::cout << "gray" << gray.type() << "binary" << binary.type() << std::endl;
